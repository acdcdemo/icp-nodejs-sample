    ...
    node('mypod') {
        ...
        container('docker') {
            stage('Build Docker Image') {
                ...
                docker build -t \${REGISTRY}/\${NAMESPACE}/icp-nodejs-sample:${env.BUILD_NUMBER} .
                """
            }
            stage('Push Docker Image to Registry') {
                ...
                docker push \${REGISTRY}/\${NAMESPACE}/icp-nodejs-sample:${env.BUILD_NUMBER}
                """
            }
        }
        container('kubectl') {
            stage('Deploy new Docker Image') {
                ...
                NAMESPACE=`cat /var/run/configs/registry-config/namespace`
                REGISTRY=`cat /var/run/configs/registry-config/registry`
                DEPLOYMENT=`kubectl --namespace=\${NAMESPACE} get deployments -l app=icp-nodejs-sample-ibm-no -o name`
